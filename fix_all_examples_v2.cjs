...
async function main() {
    ...
    for (const lvl of levels) {
        ...
        for (let i = 0; i < toFix.length; i += batchSize) {
            ...
            console.log(`Processing ${lvl} batch ${i / batchSize + 1}...`);

            const prompt = `Rewrite the example sentence for each of the following words. Provide 1 simple, short, standard example sentence for the word. Also translate it to Thai.
Input data:
