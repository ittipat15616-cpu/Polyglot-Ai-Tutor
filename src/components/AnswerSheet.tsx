  ...
  const renderMatchingDragDrop = (part: QuestionPart) => {
    ...
    return (
      <div key={part.name} className="mb-8">
        ...
        <p className="text-sm text-gray-500 mb-4">ลากตัวเลือก (A, B, C...) ไปวางในช่องว่างของแต่ละข้อ</p>
        ...
        <div className="flex flex-wrap gap-2 mb-6 p-4 bg-indigo-50 rounded-xl border border-indigo-100">
          {choices.map((choice) => (
            <div
              key={choice}
              draggable={!usedChoices.has(choice)}
              onDragStart={(e) => {
              ...
              className={`w-12 h-12 flex items-center justify-center rounded-xl font-bold text-xl shadow-sm border-2 transition-all ${
                usedChoices.has(choice) 
                  ...
                  : 'bg-white border-indigo-400 text-indigo-700 cursor-grab hover:shadow-md hover:bg-indigo-50'
              }`}
            >
              {choice}
            </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {Array.from({ length: part.endQ - part.startQ + 1 }, (_, i) => part.startQ + i).map(qNum => (
            <div key={qNum} className="flex items-center gap-3">
              ...
              <div 
                className={`w-12 h-12 rounded-xl flex items-center justify-center font-bold text-xl border-2 border-dashed transition-all ${
                  answers[qNum] 
                ...
                title={answers[qNum] ? "คลิกเพื่อยกเลิก" : "ลากมาวางที่นี่"}
              >
                {answers[qNum] || '?'}
              </div>
            </div>
