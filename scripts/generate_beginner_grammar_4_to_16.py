...
# Generate for topics 4 to 16 (index 3 to 16 in beginner_topics)
# Note: Python slice [3:16] means index 3 to 15, which corresponds to Topic 4 to 16.
for topic in beginner_topics[3:16]: 
    topic_id = topic['id']
...
async function generatePDFs() {{
  ...
  for (const topic of grammarData) {{
    try {{
      // Safe filename
      let safeTitle = topic.title.replace(/[^a-zA-Z0-9]/g, '_').substring(0, 30);
      ...
      const isSummary = topic.id.includes('Summary') || topic.id.includes('_5');
      ...
      if (topic.practice_questions && topic.practice_questions.length > 0) {{
          ...
          topic.practice_questions.forEach((q, idx) => {{
            html += `<div class="question-box"><div class="question-text">${{idx + 1}}. ${{q.question}}</div>
              ${{q.options.map(opt => {{
                const isCorrect = opt.startsWith(q.answer + ".") || opt.startsWith(q.answer + " ");
