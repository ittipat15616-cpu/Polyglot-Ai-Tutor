...
// Use VOA data since listening uses VOA content
const dataPath = path.join(__dirname, '../src/data/voa_lessons.json');
const data = JSON.parse(fs.readFileSync(dataPath, 'utf8'));

const intermediateDir = path.join(__dirname, '../public/downloads/listening/intermediate');
const advancedDir = path.join(__dirname, '../public/downloads/listening/advanced');
...
<body>
  <div class="lesson-tag">Lesson ${lessonNum} - Listening</div>
  <h1>${article.title}</h1>
  ${vocabHTML}
  ${blankQuizHTML}
  ${answerKeyHTML}
...
async function run() {
    console.log("Launching Puppeteer for Listening...");
    ...
    for (const level of levels) {
        console.log(`Processing Listening ${level.name} - ${level.articles.length} articles`);
        for (let i = 0; i < level.articles.length; i++) {
            ...
            await page.close();
            console.log(`Generated Listening ${level.name} Lesson ${lessonNum}`);
        }
    }
    
    await browser.close();
    console.log("All Listening PDFs generated successfully!");
}
