...
async function run() {
    ...
    for (const level of levels) {
        ...
        for (let i = 0; i < level.articles.length; i++) {
            ...
            // Skip if exists to save time during dev, but for production we generate all
            // We will always generate to ensure format is right.
            const html = generateHTML(article, lessonNum);
            
            const page = await browser.newPage();
            // Wait until networkidle0 to ensure Google Fonts load
