...
async function main() {
    ...
    console.log(`Need to fix: ${toFix.length} items. Will process up to 300 to stay under 60s timeout.`);
    toFix = toFix.slice(0, 300);

    const batchSize = 100; // 3 parallel calls
    const promises = [];
