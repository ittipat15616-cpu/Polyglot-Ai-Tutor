...
for (const level in cefrVocab) {
    for (const item of cefrVocab[level]) {
        ...
        if (ex.includes('/') || (!ex.endsWith('.') && !ex.endsWith('?') && !ex.endsWith('!')) || ex.split(' ').length < 3) {
            // Apply heuristic
            if (item.type.includes('noun')) {
                item.example = \`I know about \${item.word}.\`;
                item.exampleTh = \`ฉันรู้เกี่ยวกับคำว่า \${item.word}\`;
            } else if (item.type.includes('verb')) {
                item.example = \`I can \${item.word} it.\`;
                item.exampleTh = \`ฉันสามารถ \${item.word} มันได้\`;
            } else if (item.type.includes('adjective')) {
                item.example = \`It is very \${item.word}.\`;
                item.exampleTh = \`มัน \${item.word} มาก\`;
            } else if (item.type.includes('adverb')) {
...
console.log("cefrVocab updated with heuristic examples!");
